            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-800">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @foreach ($users as $user)
                        <tr><td class="px-5 py-4"><strong class="block">{{ $user->name }}</strong><span class="text-gray-500">{{ $user->email }}</span></td><td class="px-5 py-4">{{ $user->role }}</td><td class="px-5 py-4"><x-status-badge :active="$user->two_factor_enabled" :label="$user->two_factor_enabled ? 'Ativo' : 'Desativado'" /></td><td class="px-5 py-4"><x-status-badge :active="$user->active" /></td><td class="px-5 py-4"><div class="flex gap-2"><a href="{{ route('users.edit', $user) }}" class="text-brand-600">Editar</a>@if ($user->active && !$user->is(auth()->user()))<form method="POST" action="{{ route('users.deactivate', $user) }}" data-confirm-title="Inativar usuário?" data-confirm-message="O usuário perderá o acesso e todas as sessões ativas serão revogadas." data-confirm-label="Continuar" data-confirm-level="double" data-confirm-variant="danger" data-confirm-final-title="Revogar acesso deste usuário?" data-confirm-final-message="Esta é a confirmação final para inativar a conta e encerrar suas sessões." data-confirm-final-label="Sim, inativar">@csrf<button class="text-error-600">Inativar</button></form>@endif</div></td></tr>
                    @endforeach
                </tbody>
            </table>
        </div>
